...
export default function LifeInsuranceForm() {
    ...
    // derived state
    const showTermFields = form.planType === "Tem Insuarence" || form.planType === "TULIP";
    const showInvestmentFields = ["UNIP", "Child Plan", "Pension Plan", "Saving Plan"].includes(form.planType);
    ...
    const handleSubmit = async () => {
        if (!validate()) {
            Alert.alert("Error", "Please fill all the required fileds");
            return;
        };
        ...
        try {
            ...
            const leadDbId = result?.detail_lead_id;
...
/////////////////////////////////////
export default function anceFormScreen() {
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backBtn}>
